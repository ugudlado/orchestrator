...
STATE_YAML="${STATE_YAML_PATH:-${1:-}}"
if [[ -z "$STATE_YAML" ]]; then
  echo "Usage: compute-swe-metrics.sh <state_dir> (or set STATE_YAML_PATH)" >&2
  exit 1
fi
# Accept either a state.yaml path or a directory
if [[ -d "$STATE_YAML" ]]; then
  STATE_YAML="$STATE_YAML/state.yaml"
fi
