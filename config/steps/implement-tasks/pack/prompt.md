...
2. **Both `design.md` and `tasks.yaml` absent (patch schema)? Do NOT block or abandon** —
   Read `$ORCHESTRATOR_CONFIG/steps/implement-tasks/pack/reference/edge-cases.md` before
...
unrecoverable blocker? Read `$ORCHESTRATOR_CONFIG/steps/implement-tasks/pack/reference/edge-cases.md`
for the abandoned / partial completion forms.

Facing a design contradiction, missing design coverage, or scope ambiguity? See
`$ORCHESTRATOR_CONFIG/steps/implement-tasks/pack/reference/edge-cases.md` for the
