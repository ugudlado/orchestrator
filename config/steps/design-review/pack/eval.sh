...
# Design Format Contract — required sections (config/steps/design-and-draft-artifacts/pack/prompt.md § Design Format Contract)
REQUIRED_SECTIONS=(
  "## Context"
  "## Goals / Non-Goals"
...
if ! bash "$SCRIPT_DIR/../../design-and-draft-artifacts/pack/validate-tasks-yaml.sh" "$TASKS_YAML"; then
  errors=$((errors + 1))
fi
