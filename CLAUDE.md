...
- **Use `orchestrator done <state.yaml>`** with a JSON payload on stdin for all `step_history` appends. Payload contract: `config/steps/contracts/done-payload.md`. Never edit `state.yaml` directly with Write/Edit — `record.py` validates shape; direct edits have corrupted state in past runs.
