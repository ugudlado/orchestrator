...
#   - $REPO_ROOT/config/agents.yaml       (agent names for the routes side of the union)
...
# ORC-105: routes merged into config/agents.yaml.
...
if [ -n "$ALL_AGENTS_LIST" ]; then
  ...
  # ORCHESTRATOR_HOME symlinks config/ and orchestrator_next/scripts/.
