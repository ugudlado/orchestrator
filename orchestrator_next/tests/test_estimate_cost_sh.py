...
# ORC-106: orchestrator_next package moved to repo root.
# _HERE = <repo_root>/orchestrator_next/tests
# parents[0] = <repo_root>/orchestrator_next
# parents[1] = <repo_root>
_REPO_ROOT = _HERE.parents[1]             # repo root
_SCRIPTS_DIR = _REPO_ROOT / "config" / "scripts"  # estimate-cost.sh still lives here
...
def _seed_db(db_path: Path) -> None:
    ...
    orchestrator_next is a package at the repo root (ORC-106). We add the repo
    root to sys.path so the import resolves regardless of where pytest runs.
    """
