...
def test_models_prints_attributed_table(monkeypatch, tmp_path, capsys):
    """AC-5: prints TIER, TOOL, MODEL_ID, SOURCE columns per tier."""
    ...
    assert "TIER" in out and "TOOL" in out and "MODEL_ID" in out and "SOURCE" in out
    assert "opus" in out
...
def test_models_source_shows_home_override(monkeypatch, tmp_path, capsys):
    ...
    _write_models(routes_yaml, {"opus": {"tool": "cursor", "model_id": "b"}})
    _write_models(home_models, {"opus": {"tool": "claude", "model_id": "a"}})
...
def test_models_missing_config_root_falls_back_to_pack(monkeypatch, capsys, tmp_path):
    ...
    (pack / "config" / "models.yaml").write_text(
        "models:\n  sonnet: { model_id: m-1, tool: claude }\n"
    )
