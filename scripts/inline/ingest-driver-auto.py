#!/usr/bin/env python3
"""ingest-driver-auto.py — Auto-invoke orchestrator ingest-driver at complete phase.

Resolves driver session_id (TMPDIR UUID or JSONL scan) and calls
`orchestrator ingest-driver`. Exits 0 always (fail-soft per design).

Usage:  python ingest-driver-auto.py <state_yaml_path>
"""
...
def _warn(msg: str) -> None:
    print(f"[ingest-driver-auto] WARNING: {msg}", file=sys.stderr)


def main() -> int:
    if len(sys.argv) < 2:
        _warn("Usage: ingest-driver-auto.py <state_yaml_path>")
    ...
    if not session_id:
        ...
        path = "scan"
    if not session_id:
        _warn(f"session_id unresolvable for change_id={change_id}; skipping ingest-driver")
    ...
    cmd = [bin_path, "ingest-driver", "--change-id", change_id, "--session-id", session_id]
    env = dict(os.environ)
    ...
    if r.returncode != 0:
        _warn(f"ingest-driver exited {r.returncode}: {r.stderr.strip()}")
        print(json.dumps({"skipped": True, "reason": f"ingest-driver exit {r.returncode}"}))
        return 0
    ...
    print(json.dumps({"ingest_driver_result": {**ingested, "session_id": session_id, "resolution_path": path}}))
    return 0
