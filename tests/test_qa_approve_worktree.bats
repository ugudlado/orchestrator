...
setup() {
  ...
  mkdir -p "$STUB_BIN" "$FAKE_HOME" "$FAKE_ORCH/config/scripts/inline"
  : >"$MERGE_STUB_LOG"
  : >"$REMOVE_STUB_LOG"
...
write_inline_stubs() {
  cp "$ORCHESTRATOR_ROOT/config/scripts/inline/_read_state_env.sh" \
    "$FAKE_ORCH/config/scripts/inline/_read_state_env.sh"

  cat >"$FAKE_ORCH/config/scripts/inline/merge-to-main.sh" <<'STUB'
#!/usr/bin/env bash
...
STUB
  chmod +x "$FAKE_ORCH/config/scripts/inline/merge-to-main.sh"

  cat >"$FAKE_ORCH/config/scripts/inline/remove-worktree.sh" <<'STUB'
#!/usr/bin/env bash
...
STUB
  chmod +x "$FAKE_ORCH/config/scripts/inline/remove-worktree.sh"
}
